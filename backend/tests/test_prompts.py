...
def _create(client, headers, content, name="system", activate=True):
    resp = client.post(
        "/api/prompts",
        headers=headers,
        json={"name": name, "kind": "system", "content": content, "activate": activate},
    )
...
def test_rollback_by_activating_older_version(client, auth_headers):
    prompts = client.get("/api/prompts", headers=auth_headers).json()
    system_versions = sorted((p for p in prompts if p["name"] == "system"), key=lambda p: p["version"])
...
def test_prompt_edits_are_audited(client, auth_headers):
    ...
    entries = client.get("/api/audit", headers=auth_headers, params={"action": "prompt_edited"}).json()
...
def test_prompts_require_admin(client, auth_headers):
    client.post(
        "/api/users",
        headers=auth_headers,
        json={"name": "Pm", "email": "pm@test.com", "password": "secret123", "role": "manager"},
    )
    login = client.post("/api/auth/login", json={"email": "pm@test.com", "password": "secret123"})
