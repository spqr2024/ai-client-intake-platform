...
NODE_LIBRARY: list[dict] = [
    ...
    },
    {
        ...
        "field": "client_phone",
        "type": "phone",
        "prompt": {"en": "What's the best phone number to reach you?",
    ...
    },
    {
        ...
        "options": {
            ...
            "uk": ["Вебсайт", "Інтернет-магазин", "Мобільний застосунок",
                   "Брендинг / Дизайн", "Інше"],
        },
    },
    {
        "key": "budget",
        ...
        "field": "budget",
        "type": "number",
        "prompt": {"en": "What's your approximate budget for this project (in USD)?",
                   "uk": "Який ваш орієнтовний бюджет на цей проєкт (у доларах США)?"},
        "options": {"en": ["$1000", "$2000", "$5000", "$10000+"],
    ...
    },
    {
        ...
        "field": "timeline",
        "type": "text",
        "prompt": {"en": "When would you like the project completed by?",
                   "uk": "До якого терміну ви хотіли б завершити проєкт?"},
        "options": {"en": ["ASAP", "Within 1 month", "1-3 months", "Flexible"],
    ...
    },
    {
        ...
        "field": "goals",
        "type": "text",
        "prompt": {"en": "Could you describe your project goals in a sentence or two?",
    ...
    },
    {
        ...
        "field": "extra_notes",
        "type": "text",
        "prompt": {"en": "Is there anything else important we should know?",
    ...
    },
    {
        ...
        "definition": {
            ...
            "nodes": {
                ...
                "email": {
                    "field": "client_email", "type": "email", "skip_if_known": True,
        ...
        "definition": {
            ...
            "nodes": {
                ...
                "reason": {
                    ...
                    "options": {"en": ["New symptoms", "Follow-up visit", "Routine check-up",
                                       "Prescription refill"]},
                    "next": "symptoms",
                },
                "symptoms": {
    ...
    },
    {
        ...
        "definition": {
            ...
            "nodes": {
                ...
                "company_size": {
                    "field": "company_size", "type": "choice",
...
def analyze(definition: dict) -> dict:
    ...
    if not nodes:
        return {"reachable": [], "unreachable": [], "warnings": ["Flow has no steps"],
                "terminal_nodes": [], "has_cycle": False}
    ...
    if unreachable:
        warnings.append(
            f"{len(unreachable)} step(s) can never be reached: {', '.join(unreachable)}"
        )

    # Terminal nodes (empty `next` and no branches) end the flow and create the lead.
    terminal = sorted(
        node_id for node_id, node in nodes.items()
        if not node.get("next") and not node.get("branches")
    )
    if not terminal:
