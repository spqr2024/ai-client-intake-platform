...
DEMO_LEADS = [
    (
        "Online store — Alice Johnson",
        "Alice Johnson",
        "alice@brightcandle.com",
        "Online store",
        5000,
        "1-3 months",
        "Qualified",
    ...
    (
        "Вебсайт — Катерина Шевченко",
        "Катерина Шевченко",
        "kateryna@salonlux.ua",
        "Вебсайт",
        2000,
        "Якнайшвидше",
        "Qualified",
    ...
    ("Website — (anonymous)", "", "", "Website", None, "", "Incomplete", 15, "Low", [], 6, "", "en", None),
    (
        "Mobile app — Tomas Novak",
        "Tomas Novak",
        "tomas@logistix.cz",
        "Mobile app",
        18000,
...
TRANSCRIPT_TEMPLATE = [
    (
        "bot",
        "Hello! I'm your intake assistant — I'll gather a few project details so our team "
        "can help you faster. May I have your name?",
        "greeting",
        "name",
    ),
    ...
    ("bot", "Could you describe your project goals or needs in a sentence or two?", "question", "goals"),
...
def _already_provisioned(db: Session, workspace_id: int) -> bool:
    return bool(db.scalar(select(func.count(Lead.id)).where(Lead.workspace_id == workspace_id)))
...
def _ensure_kb(db: Session, workspace_id: int) -> None:
    existing = db.scalar(
        select(func.count(KnowledgeBaseArticle.id)).where(KnowledgeBaseArticle.workspace_id == workspace_id)
    )
    if existing:
        return
    ...
    for index, node in enumerate(("service", "budget", "budget", "email")):
        ...
        db.add(
            Message(
                ...
                meta={"node": "service", "event": "greeting"},
                created_at=started,
            )
        )
        db.add(
